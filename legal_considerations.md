...
> **This document is synced across three repositories:** [`poe_mcp_suite`](https://github.com/charleslucas/poe_mcp_suite) (the canonical copy), [`poe-skilltree-export`](https://github.com/charleslucas/poe-skilltree-export) (this repo), and [`poe-atlastree-export`](https://github.com/charleslucas/poe-atlastree-export). The version in `poe_mcp_suite/legal_considerations.md` is the source of truth — if you update one, sync to all three.

This document captures the reasoning behind how this fork and the related `poe_mcp_suite` use Path of Exile game data — what we extract, what we redistribute, what we deliberately don't. It exists so that anyone (Grinding Gear Games, a curious contributor, an opportunistic lawyer, future maintainers) can see we thought about it.
